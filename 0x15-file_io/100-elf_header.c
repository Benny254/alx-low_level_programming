 ...
 * @e_ident: The pointer to an array containing a ELF magic numbers.
 *
...
void print_magic(unsigned char *e_ident)
{
	...
	for (value = 0; value < EI_NIDENT; value++)
	{
		printf("%02x", e_ident[value]);

		if (value == EI_NIDENT - 1)
			printf("\n");
		else
			printf(" ");
	}
 ...
 * @e_ident: The pointer to an array containing an ELF class.
 */
void print_data(unsigned char *e_ident)
{
	printf(" Data: ");
 ...
 *   * @e_ident: The pointer to an array containing an ELF version.
 *    */
void print_version(unsigned char *e_ident)
{
	 printf(" Version: %d",
